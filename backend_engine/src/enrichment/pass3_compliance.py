...
async def run_pass3(db: Client, legal_doc_id: str) -> None:
        ...
        try:
            ...
            for result in results:
                ...
                await db.structuralunit.update(
                    ...
                    data={"provision_nature": json.dumps(nature_tags)},
                )
                ...
                for ob in res_dict.get("obligations", []):
                    created_ob = await db.obligationrecord.create(
                        data={
                            ...
                            "compliance_category": ob.get(
                                "compliance_category", "ongoing"
                            ),
                    ...
                    for pen in ob.get("penalties", []):
                        await db.penaltyrecord.create(
                            data={
                                ...
                                "fine_amount": str(pen.get("fine_amount", ""))
                                if pen.get("fine_amount")
                                else None,
                                "imprisonment": pen.get("imprisonment"),
                                "liable_person": pen.get(
                                    "liable_person", ob.get("responsible_person")
                                ),
    ...
    logger.info(
        f"[Pass 3] Processed {processed_count} units, {obligation_count} obligations."
    )
