...
async def run_pass4(db: Client, legal_doc_id: str) -> None:
    ...
    for unit in units:
        ...
        for match in INTRA_DOC_PATTERN.finditer(text):
            ref_typeword = (
                text[match.start() : match.span()[0] + 15].split()[0].lower()
            )  # hacky way to get word
            ...
            target = await db.structuralunit.find_first(
                where={
            ...
            await db.crossreference.create(
                data={
                    ...
                    "target_section_ref": f"{ref_typeword} {ref_val}"
                    if target is None
                    else None,
                }
            )
        ...
        for match in INTER_DOC_PATTERN.finditer(text):
            ...
            target_doc = await db.legaldocument.find_first(
                where={
                    "title": {
                        "contains": target_act_name.split(",")[0],
                        "mode": "insensitive",
                    },
                    "status": "active",
                }
