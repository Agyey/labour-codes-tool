...
from src.redis_client import redis_db
...
async def trigger_analysis(
    ...
    if doc.status != "analyzed" and doc.status != "analyzing":
        await db.document.update(
            where={"id": document_id}, data={"status": "analyzing"}
        )
        process_document_task.delay(document_id, doc.raw_text)
...
async def _stream_reader(document_id: str) -> typing.AsyncGenerator[str, None]:
    ...
    history = await redis_db.lrange(history_key, 0, -1)  # type: ignore # mypy has issues with redis async union return types
    for evt in history:
        yield evt

    doc = await db.document.find_unique(where={"id": document_id})
    if not doc or (doc.status != "analyzing" and not history):
        return

    # Subscribe to incoming live events using Redis PubSub
...
async def stream_analysis(request: Request, document_id: str) -> StreamingResponse:
    ...
    # Only start background task if not already tracked by Celery (status == analyzing handles this)
    if doc.status != "analyzed" and doc.status != "analyzing":
        await db.document.update(
            where={"id": document_id}, data={"status": "analyzing"}
        )
