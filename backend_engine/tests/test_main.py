...
def test_upload_document_no_filename(client):
    response = client.post("/api/documents/upload", files={"file": ("", io.BytesIO(b""), "application/pdf")})
    assert response.status_code in (400, 422)

def test_upload_document_bad_extension(client):
    response = client.post("/api/documents/upload", files={"file": ("test.exe", io.BytesIO(b""), "application/octet-stream")})
...
def test_upload_document_too_large(mock_settings, client):
    ...
    response = client.post("/api/documents/upload", files={"file": ("test.pdf", io.BytesIO(b"A" * 1024 * 1024), "application/pdf")})
    assert response.status_code == 413

@patch("src.main.extract_text_from_pdf")
def test_upload_document_exception(mock_extract, client):
    mock_extract.side_effect = Exception("test error")
    response = client.post("/api/documents/upload", files={"file": ("test.pdf", io.BytesIO(b"PDF content"), "application/pdf")})
...
def test_approve_suggestion_not_pending(mock_db, client):
    class MockSuggestionDone:
        status = "approved"
    mock_db.documentsuggestion.find_unique = AsyncMock(return_value=MockSuggestionDone())
...
def test_approve_suggestion_legislation_no_framework(mock_db, client):
    ...
    mock_db.documentsuggestion.find_unique = AsyncMock(return_value=MockSuggestionLegis())
...
def test_approve_suggestion_all_types(mock_audit, mock_db, client):
    ...
    for t_idx, (t_name, t_query) in enumerate(types):
        class MockSuggestionAny:
            ...
            type = t_name
            target_module = "tm"
            suggested_data = {"name": "n", "short_name": "sn", "year": "2024", "task": "t", "due_logic": "d"}
        
        mock_db.documentsuggestion.find_unique = AsyncMock(return_value=MockSuggestionAny())
