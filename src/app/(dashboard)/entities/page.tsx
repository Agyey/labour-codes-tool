  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Entities", value: "48", icon: Building2, color: "text-slate-600" },
          { label: "Healthy", value: "32", icon: CheckCircle2, color: "text-emerald-500" },
          { label: "Warnings", value: "12", icon: AlertTriangle, color: "text-amber-500" },
          { label: "Critical", value: "4", icon: XCircle, color: "text-rose-500" },
        ].map((stat, i) => (
                ...
                <tr key={ent.id} className="hover:bg-slate-50/80 dark:hover:bg-zinc-800/30 transition-colors group cursor-pointer">
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{ent.type}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="w-32 space-y-1.5">
                      <div className="flex justify-between text-[10px] font-bold">
                        <span className="text-slate-500">{ent.health}%</span>
                      </div>
                      <div className="h-1.5 w-full bg-slate-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full ${ent.health > 90 ? 'bg-emerald-500' : ent.health > 70 ? 'bg-amber-500' : 'bg-rose-500'}`}
                          style={{ width: `${ent.health}%` }}
                        />
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-zinc-400">
                      <History className="w-3.5 h-3.5" /> {ent.lastAudit}
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <span className={`px-2 py-1 rounded-lg font-black text-xs ${
                      ent.complianceScore.startsWith('A') ? 'bg-emerald-50 text-emerald-600' :
                      ent.complianceScore.startsWith('B') ? 'bg-amber-50 text-amber-600' :
