...
export function AppSidebar() {
  ...
  return (
    <aside className="w-56 flex-shrink-0 border-r border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 min-h-[calc(100vh-120px)] overflow-y-auto transition-colors duration-300">
      <div className="p-3">
        ...
        <button
          ...
          className="w-full text-left px-2.5 py-1.5 rounded-md text-xs mb-0.5 transition-colors cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-800 dark:text-zinc-300 dark:hover:text-white"
        >
          All Chapters
        </button>
        ...
        {chapters.map(([ch, name]) => (
          <button
            ...
            className="w-full text-left px-2.5 py-1.5 rounded-md text-xs mb-0.5 transition-colors cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-800 group"
          >
        ...
        <div className="mt-4 pt-4 border-t border-slate-200 dark:border-zinc-800">
          <h4 className="text-xs font-bold text-slate-700 dark:text-zinc-300 mb-2 flex items-center gap-1">
            <Pin className="w-3 h-3 text-amber-500" />
            Pinned
          </h4>
          {pinnedProvisions.length === 0 ? (
            <p className="text-[10px] text-slate-400 dark:text-zinc-600">No pins yet</p>
          ) : (
            pinnedProvisions.map((p) => (
              <button
                ...
                className="w-full text-left px-2 py-1 rounded text-[10px] cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors block"
                style={{ color: cObj.c }}
              >
