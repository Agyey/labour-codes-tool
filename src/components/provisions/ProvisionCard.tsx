...
export const ProvisionCard = memo(function ProvisionCard({ provision: p }: ProvisionCardProps) {
  ...
  return (
    <div 
      id={`prov-${p.id}`}
      className={`group relative bg-white rounded-2xl border ${isExpanded ? 'border-slate-300 ring-4 ring-slate-100 shadow-xl' : 'border-slate-200 hover:shadow-lg hover:border-slate-300'} transition-all duration-300 overflow-hidden mb-4`}
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <div className="flex items-center gap-1">
              {(p.workflowTags || []).map(t => (
                <Badge key={t} text={t} color={WORKFLOW_TAG_COLORS[t] || "#64748b"} />
              ))}
            </div>
          </div>
        </div>
